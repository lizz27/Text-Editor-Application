...
public class MarkovTextGeneratorLoL implements MarkovTextGenerator {
	...
	public void train(String sourceText)
	{
		...
		for (int i = 1; i < sourceTextList.length; i++) {
			if (getListNode(prevWord) != null) {
				getListNode(prevWord).addNextWord(sourceTextList[i]);
			} else {
				ListNode newNode = new ListNode(prevWord);
				newNode.addNextWord(sourceTextList[i]);
				wordList.add(newNode);
			}
			prevWord = sourceTextList[i];
		}
	 ...
	 * 1) If word is in wordList, return ListNode
	 * 2) If word is not in wordList, return null
	 *  */
	...
	public String generateText(int numWords) {
		...
		while (i < numWords) {
			ListNode currNode = getListNode(currWord);
